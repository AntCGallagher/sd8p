...
# Adjusted to get distortion data for each pitch
def get_radial_data(pitch=0, filename=PATH+'/calibrations/undistort_pitch'):
    full_filename = filename + str(pitch) + '.txt'
    _file = open(full_filename, 'r')
    data = cPickle.load(_file)
    _file.close()
    return data[pitch]

def get_perspective(pitch=0, filename=PATH+'/calibrations/perspective.json'):
