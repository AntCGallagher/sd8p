...
class MyTracker(object):
    ...
    def recognize_plates(self, image, robot_mask, ball_mask, draw=True, show_window=True):
        ...
        for cnt in contours:
            ...
            if blue_yellow_ration < 0.5:
                #pink
                min_range = self.calibration['blue']['min']
                max_range = self.calibration['blue']['max']
            else:
                #green
                min_range = self.calibration['yellow']['min']
                max_range = self.calibration['yellow']['max']
            ...
            # find the mass centre of the other single circle (to find angle)
            if blue_yellow_ration < 0.5:
                #blue
                min_range = self.calibration['blue']['min']
                max_range = self.calibration['blue']['max']
            ...
            # get direction
            direction_vector_x = -(cx % 4][0] - box[(closest_corner + 1) % 4][0])
