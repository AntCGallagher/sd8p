...
void HMC5883L::initCompass(){
    ...
    int error = setScale(float(1.3));                              // Set the scale of the compass.
    
    if(error == ERRORCODE_1_NUM){                                         // If there is an error, print it out.
        Serial.println(getErrorText(error));
    }
    else
      Serial.println("Set compass scale to 1.3.");
    
    setMeasurementMode(MEASUREMENT_CONTINUOUS);     // Set the measurement mode to Continuous
    Serial.println("Set compass measurement mode to continous.");
